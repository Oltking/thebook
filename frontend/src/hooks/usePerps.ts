...
export function usePerps() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setMarks({ BTC: Number(b) / CENTS, ETH: Number(e) / CENTS, VARA: Number(v) / CENTS });
      const res: any = await program.perps.getReserve().call();
      setReserve(Number(res) / CENTS);
      if (account) {
        ...
        for (const p of raw || []) {
          ...
          parsed.push({
            ...
            entry: Number(p[3]) / CENTS,
            margin: Number(p[4]) / CENTS,
            leverage: Number(p[5]),
            pnl: Number(p[6]) / CENTS,
            liqPrice: Number(liqRaw) / CENTS,
          });
        }
        setPositions(parsed);
      } else {
  ...
  const openPosition = useCallback(async (asset: Asset, isLong: boolean, marginUsd: number, leverage: number): Promise<string | null> => {
    const marginCents = BigInt(Math.round(marginUsd * CENTS));
    const err = await send(() => program!.perps.openPosition(asset, isLong, marginCents.toString(), leverage));
